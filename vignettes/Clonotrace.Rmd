...
Understanding clonal structure and dynamics is a central problem in single-cell transcriptomics, particularly in immune and cancer biology. Modern single-cell technologies increasingly enable joint measurement of transcriptomes and clonal barcodes (e.g., TCR sequences or genetic lineage tracers), providing an opportunity to study how transcriptional states evolve at the clone level rather than at the level of individual cells.

**Clonotrace** is designed to analyze clonal behavior in single-cell data by integrating transcriptomic embeddings with clone information. Rather than treating clones as discrete labels, Clonotrace models each clone as a distribution of cells in a low-dimensional embedding space, enabling quantitative comparison of clonal transcriptional states.

This vignette provides a step-by-step tutorial demonstrating how to use Clonotrace to:
- Estimate clone-level density in a cell embedding
- Compute clone-to-clone distances using optimal transport (OT)
- Cluster clones into transcriptional profiles
- Perform downstream analyses such as profile-level pseudotime and gene association

---

## Installation

Clonotrace can be installed from GitHub:

```{r,eval = FALSE}
install.packages("devtools")
devtools::install_github("yuntianf/Clonotrace")
```

### data
Clonotrace requires two main inputs:

A low-dimensional cell embedding, typically derived from PCA, or other dimensionality reduction methods applied to gene expression data.

A cell-by-clone identity matrix, which is a binary matrix to indicate if a given cell belongs to a given clone.

Throughout this vignette, we assume that the embedding has already been computed and that clone identity are available (e.g., from TCR sequence or lineage barcodes).


We begin by loading an example dataset. The example data is from a public in vitro longitudinal experiment to monitor the hematopoiesis (https://www.science.org/doi/10.1126/science.aaw3381). You can download the demo data from: https://upenn.box.com/s/x099vybef0qop0cljbuan89x5jqvgnwx
...
This data has a simple cell type composition with only 3 cell types:
...
For a higher resolution in the downstream analysis, we further cluster cells into 9 cell clusters.
...
We first build a cell kNN graph based on the PCA space:
...
Then we could spread the clone label in the cell graph to smooth the cell density in those expanding clones. There is a bootstrap during this process to filter out propagations with high deviance, usually in regions with high density of lowly expanded or non-expanded clones. So it's better to run this in parallel.
...
There are two main outputs from this step, the first one is the propagated clone labels for each cell and the second one is the deviance of each propagated label.
...
We can first show the distribution of the deviance in a histogram:
...
We can visualize the deviance of clone label spreading in each cell via cell UMAP, and here we use the 0.3 as the upper bound threshold for deviance. Cells with high deviance are enriched in undifferentiated cells.
...
Then we filter out cells with high deviance, and sparsify the cell-to-clone probability by keeping top 90% mass
...
First we show the raw cell distribution of the example clone:
...
And after label propagation, its cell distribution becomes more continous.
...
After we smooth the cell density for each expanded clone over the cell transcriptomic embedding, we can measure their cell distributional difference via optimal transportation distance. 

We provide two methods for this, the first one calculates the exact optimal transportation distance, and the second one is a faster approximation, but less accurate. You can switch the modes by the `exact` parameter in the `clone_disance` function.
...
Here for time efficiency, we can use the pre-computed clone-wise distance for downstream applications.

```{r}
file_path <- system.file("extdata", "clone_graph_dis.tsv", package = "Clonotrace")
clone_dis = read.table(file_path,sep = "\t")

clone_dis = long2square(long = as.data.frame(clone_dis),row_names_from = "group1",
                        col_names_from = "group2",values_from = "dis",symmetric = T)
diag(clone_dis) = 0
...
We can also visualize the clone embedding in other low dimension reductions like a diffusion map:
...
Diffusion map is good to layout continuous movement of clones over time. Here we can mark how clones with the same lineage barcodes change across time.
...
From the diffusion map we can see an organized clonal shift from day4 to day6.

### profile projection in cell embedding

After we cluster the clones into profiles, with the cell clone probability, we can also map cells to profiles:
...
Obvisouly, profile 5 and 6 unipotently differentiate to monocytes and profile 3 and 7 unipotently differentiate to netriphil, while profile 1 and 4 denote the bipotent differenitation.
...
This function `cell_clone_coembed` would return a sparse distance matrix, which denotes a weighted kNN network between cells. We can project this kNN network to UMAP for visualization. This step requires a python script.
...
Obviously, there appear two boundaries within the two branches for monocytes (cluster 4, 3, 5) and neutriphils (cluster 1, 6, 7), which denote the clone level heterogeneity. Once we colored cells by the profile identity, we can see the two boundaries seperate the bipotent and unipotent profiles.
...
In the downstream analysis we will focus on the early bipotent profiles (profile) and detect early fate-associated genes. To achieve this goal we first do an enrichment test between profiles and clusters to show in which cluster profile 1 has significant high density. And then within the target cluster, we compare the gene expression between profile 1 and other profiles to identify early fate-associated genes.
...
From the heatmap we can see that our target profile 1 is enriched in 4 cell clusters, including 1, 3, 4 and 8, within which cluster 1 and 4 are at early differentiation stage. Here, as an example, we will identify genes differentially expressed in profile 1 in cluster 4.
...
Obviously, this `Dmkn` gene shows higher expression in the bipotent profile no matter in monocyte or neutriphil.
